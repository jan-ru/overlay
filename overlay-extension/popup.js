...
async function createSmartOverlay(color) {
  ...
  try {
    await executeScriptInActiveTab((overlayColor) => {
      ...
      // Inline the calendar detection function
      function findCalendarContainer() {
        console.log('🔍 Looking for calendar container...');
      ...
      Object.assign(overlay.style, {
        ...
        zIndex: '999999',
        pointerEvents: 'none',
        border: '3px solid red', // More visible border for debugging
        boxSizing: 'border-box',
        opacity: '1' // Ensure opacity is set
      });
      
      // Add some debugging info
      overlay.setAttribute('data-debug', `w:${dimensions.width} l:${dimensions.left} color:${overlayColor}`);
      
      // Log the computed styles after creation
      ...
      // Additional debugging - check if element exists and is visible
      const checkOverlay = document.getElementById('custom-overlay');
      if (checkOverlay) {
        const rect = checkOverlay.getBoundingClientRect();
        const computedStyles = window.getComputedStyle(checkOverlay);
        console.log('📐 Overlay verification:', {
          ...
          parentElement: checkOverlay.parentElement?.tagName
        });
        
        // Try to make it super visible
        checkOverlay.style.backgroundColor = 'rgba(255, 0, 0, 0.8)'; // Bright red with high opacity
        checkOverlay.style.border = '5px solid yellow';
        console.log('🚨 Made overlay super visible for testing');
      } else {
        console.error('❌ Overlay element not found after creation!');
      }
    ...
    console.log('🔄 Falling back to standard overlay');
